...
class ExpenseCategory extends BaseModel {
  ...
  static async create(payload) {
    const result = await query(
      `INSERT INTO expense_categories (category_name, category_code, description, is_active)
       VALUES (?, ?, ?, ?)`,
      [payload.category_name, payload.category_code, payload.description || null, normalizeBoolean(payload.is_active)]
    );
  ...
  static async list({ limit, offset, is_active, search }) {
    ...
    if (search) {
      conditions.push("(category_name LIKE ? OR category_code LIKE ?)");
  ...
  static async update(id, payload) {
    ...
    await query(
      `UPDATE expense_categories
       SET category_name = ?, category_code = ?, description = ?, is_active = ?
       WHERE id = ?`,
      [
        payload.category_name !== undefined ? payload.category_name : existing.category_name,
        payload.category_code !== undefined ? payload.category_code : existing.category_code,
